...
# -*- coding: utf-8 -*-
"""
Evaluate fine-tuned Rex-Omni model on OV-IGOD test set - Per-Affordance AP metrics

This script evaluates each affordance separately (like PF-Florence approach),
then computes mean AP across all affordances.

Usage:
    python evaluate_ovigod_ap_per_affordance.py --checkpoint finetuning/work_dirs/ovigod_sft-1000/checkpoint-93 --max_samples 100
"""
...
def compute_iou(box1, box2):
    ...
    try:
        if isinstance(box1, (list, tuple)) and len(box1) == 4:
            ...
            print(f"Warning: Invalid box1 format: {box1}, type: {type(box1)}")
            return 0.0
        ...
        if isinstance(box2, (list, tuple)) and len(box2) == 4:
            ...
            print(f"Warning: Invalid box2 format: {box2}, type: {type(box2)}")
            return 0.0
    except (TypeError, ValueError) as e:
        print(f"Error converting box coordinates: box1={box1}, box2={box2}, error={e}")
        return 0.0
...
def evaluate_at_iou_threshold(all_predictions, all_ground_truths, iou_threshold):
    ...
    for sample_id in all_ground_truths.keys():
        ...
        # Add confidence to each prediction
...
def main():
    ...
    print("="*80)
    print("OV-IGOD Test Set Evaluation - Per-Affordance AP Metrics")
    print("="*80)
    ...
    per_affordance_predictions = defaultdict(dict)  # affordance -> {sample_id -> [boxes]}
        ...
        for sample_info in batch_samples:
            try:
                ...
                for box in sample_info['target_bboxes']:
                    ...
                    # Ensure it's a proper list of floats
            ...
            try:
                ...
                for result, metadata in zip(results, batch_metadata):
                    ...
                    for pred in predictions.get(affordance, []):
                        coords = pred['coords']
                        # Ensure coords is a flat list of 4 numbers
                        if isinstance(coords, (list, tuple)) and len(coords) == 4:
                            try:
                                ...
                                print(f"Warning: Cannot convert coords to float: {coords}, error: {e}")
                        else:
                            print(f"Warning: Sample {sample_id} has malformed prediction box: {coords}")
    ...
    print(f"Total GT count: {total_gts}")
    ...
    # print(f"\n{'Affordance':<20} {'AP@50':>10} {'AP@75':>10} {'AP@50:95':>10}")
    # print("-" * 52)
    ...
    results_dict = {
        ...
        "evaluation_type": "per_affordance",
